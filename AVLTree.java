 ...
 * Class 'LUC_AVLTree'
 *
 * LUC_AVLTree class definition for constructing / accessing / modifying the AVL
 * tree. In order to provide a general purpose AVL tree, better error handling
 * should to be provided.
 *
 * Notes: 1. This AVL tree does not allow duplicates, if one is attempted to be
 * inserted, no action is taken by the method (though an error should be
 * thrown). 2. If a deletion is attempted for a non-existent value, no action is
 * taken (again, returning an error indicator and/or throwing an error would be
 * appropriate.
 *
 * Public methods: void removeAll() - Remove all nodes of the tree (empties
 * tree) boolean checkEmpty() - Returns boolean value if tree is empty or not
 * void insert(int value) - inserts 'value' into the tree void delete(int value)
 * - removes 'value' from the tree String preorderTraversal() - returns a
 * preorder traversal of tree in a String
 *
 */
class LUC_AVLTree {
     ...
     *
     * isTreeBalanced() is private method which uses recursion based on preorder
     * traversal to determine if an AVL [sub]tree is balanced. For an AVL tree
     * to be balanced, every node must have a |bf| <= 1
     *
     ...
     * represents the node's height in the tree. Think of the node's height as
     * the number of edges to the deepest leaf of that node.
     *
     ...
     *
     * Additionally, as this method returns through recursion, it 1. re-adjusts
     * the 'height' variable in each ancestor's node. And, 2. returns the
     * current node. If a rotation is done, then this top node will change. This
     * allows, through recursion, the invoker to readjust the parent's child
     * pointer to this returned node.
     *
     ...
     *
     * deleteElement() is private method that uses recursion. It will delete an
     * element within the tree. It considers several scenarios on where the
     * element to delete is in the tree.
     *
     * This includes the following scenarios of where the node to delete is in
     * the tree: 1. leaf node - simpliest case, just return null (which removes
     * node) 2. interior node with only left subtree below it (node gets
     * replaced with left subtree) 3. interior node with only right subtree
     * below it (node gets replaced with right subtree) 4. interior node with
     * both a left and right subtree below it. In this scenario, it gets the
     * inorder successor node (aka, the smallest value node in the right
     * subtree, this is accomplished by using method minValueNode()). Once
     * found, it then copies the inorder successor node's value to this node,
     * which for all purposes deletes the node. Finally, it needs to delete that
     * inorder successor node from the right subtree.
     *
     ...
     *
     * In scenario 4, the routine pulls up (and deletes) the "Inorder Successor"
     * node on its right subtree. It will be a leaf node.
     *
     * This node (and its ancestors nodes) may need re-balancing; so the bf is
     * checked at the end of this routine and if needing re-balancing, it
     * invokes the proper rotation (LL, LR, RR, RL).
     *
     * Again, the heights for each of its ancestor nodes are re-recalculate to
     * the root.
     *
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (value < node.value) {
            //go left in tree to find node with value
            node.leftChild = deleteElement(value, node.leftChild);
        } else if (value > node.value) {
            //go right in tree to find node with value
            node.rightChild = deleteElement(value, node.rightChild);
        } else {
            //check if one child or no child
            if (node.leftChild == null || node.rightChild == null) {
                //find if one child which one is null, or make temp null if no children
                Node temp = (node.leftChild != null) ? node.leftChild : node.rightChild;
                return temp;
            } else {
                //else find minumum value to replace node
        ...
        //Start checking if balancing is needed and if so which one
     ...
     * Method getBalance
     *
     * getBalance() is a private method that returns the balance factor (bf) of
     * a node. If the return value is within the set {1, 0, -1} then it is
     ...
     * Method minValueNode
     *
     * minValueNode() is a private method that given a node, it will return a
     * pointer to the node that represents the minimum value on its left
     ...
     * Method: LLRotation
     *
     * LLRotation() is a private method that performs a rotation where node X is
     * the node out-of-balance due to Left Left children. After the rotation,
     ...
     * X
     *             / \
     * Y xr Y / \ ==> / \ Z yr Z X / \ / \ / \ zl zr zr zl yr xr
     *
     ...
     * Method: LRRotation
     *
     * LRRotation() is a private method that performs a rotation where node X is
     * the node out-of-balance due to Left Right children. After the rotation,
     * node X's left Right grand-child, Z, becomes the new top of the [sub]tree.
     * And, Z's left sub-child, zl, becomes the right sub-child of Y, and right
     * sub-child, zr, becomes the left sub-child of X. Node Z is returned as the
     * new top to the [sub]tree. The height values of nodes X, Y, and Z are
     * adjusted, each changes in this rotation.
     *
     * X
     *        / \
     * Y xr Z / \ ==> / \ yl Z Y X / \ / \ / \ zl zr yl zl zr xr
     *
     * @param X - node that is out of balance
     *
     ...
     * Method: RRRotation
     *
     * RRRotation() is a private method that performs a rotation where node X is
     * the node out-of-balance due to Right Right children. After the rotation,
     * its right sub-child, Y, becomes the new top of this [sub]tree. And, node
     * Y's left sub-child, yl, becomes the right sub-child of X. Node Y is
     * returned as the new top to the [sub]tree. The height values of nodes X
     * and Y are re-adjusted. Z's height does not change.
     *
     * X
     *             / \
     * xl Y Y / \ ==> / \ yl Z X Z / \ / \ / \ zl zr xl yl zl zr
     *
     * @param X - node that is out of balance
     *
     * @return Y - new top node of [sub]tree after rotation
     ...
     * X
     *        / \
     * xl Y Z / \ ==> / \ Z yr X Y / \ / \ / \ zl zr xl zl zr yr
     *
     * @param X - node that is out of balance
     *
     * @return Z - new top of the [sub]tree after rotation
